...
export default function BattleScreen({
  ...
  const handlePlayerComicDismiss = useCallback(() => {
    ...
    if (turn.damageToPlayer > 0) entries.push({ type: "effect", text: `  💥 You take ${turn.damageToPlayer} damage!` });
    if (turn.playerEffect && turn.playerEffect !== "none")
      entries.push({ type: "effect", text: `  ${EFFECT_ICONS[turn.playerEffect] || "✦"} You are affected by ${turn.playerEffect}!` });
  ...
  const handleEnemyComicDismiss = useCallback(() => {
    ...
    setLog((prev) => [...prev, { type: "system" as const, text: `─── Turn ${updatedBattle.turn}/${updatedBattle.maxTurns} ───` }]);
    setComicPhase("idle");
    setAllPanels([]);
    setPendingTurn(null);
  }, [pendingTurn]);
  ...
  const showTurnAsTextOnly = useCallback((turn: PendingTurn["turn"], updatedBattle: BattleState) => {
    ...
    if (turn.playerEffect && turn.playerEffect !== "none")
      entries.push({ type: "effect", text: `  ${EFFECT_ICONS[turn.playerEffect] || "✦"} You are affected by ${turn.playerEffect}!` });
    entries.push({ type: "system", text: `─── Turn ${updatedBattle.turn}/${updatedBattle.maxTurns} ───` });
    setLog((prev) => [...prev, ...entries]);
    setBattle(updatedBattle);
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/battle/turn", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ battleId, prompt: playerPrompt }),
      });
      ...
      if (turn.damageToEnemy > 0)
        playerEntries.push({ type: "effect", text: `  ⚔ ${turn.damageToEnemy} damage to ${enemyName} [creativity: ${turn.creativityScore}/10]` });
      if (turn.enemyEffect && turn.enemyEffect !== "none")
        playerEntries.push({ type: "effect", text: `  ${EFFECT_ICONS[turn.enemyEffect] || "✦"} ${enemyName} is affected by ${turn.enemyEffect}!` });
      ...
      setError(err instanceof Error ? err.message : "The Game Master is distracted...");
  ...
  return (
    <div className="fade-in flex flex-col" style={{ height: "calc(100vh - 2rem)" }}>
      {comicPhase === "player-comic" && allPanels.length === 4 && (
        <BattleComic panels={[allPanels[0], allPanels[1]]} labels={["YOUR ATTACK", "IMPACT!"]} onDismiss={handlePlayerComicDismiss} />
      )}
      {comicPhase === "enemy-comic" && allPanels.length === 4 && (
        <BattleComic panels={[allPanels[2], allPanels[3]]} labels={["ENEMY STRIKES", "AFTERMATH"]} onDismiss={handleEnemyComicDismiss} />
      )}

      <div className="mb-4 p-3 border border-[var(--green-dim)] shrink-0">
        <div className="flex justify-between items-center mb-2">
          <span className="text-cyan glow-cyan">YOU</span>
          <span className="text-dim text-sm">Turn {battle.turn}/{battle.maxTurns}</span>
          <span className="text-red glow-red">{enemyName}</span>
        </div>
        <HpBar current={battle.playerHp} max={100} label="PLAYER" color="player" />
      ...
      <div className="flex-1 overflow-y-auto p-3 border border-[var(--green-dim)] mb-4 min-h-0">
        ...
        {submitting && <div className="text-amber glow-amber"><span className="typing-dots">The Game Master deliberates</span></div>}
        {imageLoading && !showingComic && <div className="text-cyan text-sm"><span className="typing-dots">Generating battle vision</span></div>}
        <div ref={logEndRef} />
      </div>

      {battle.turn === 0 && !submitting && comicPhase === "idle" && (
        <div className="mb-3 text-sm shrink-0">
          <p className="text-dim mb-1">Example prompts:</p>
      ...
      {battle.status === "active" && comicPhase === "idle" && (
        <form onSubmit={handleSubmit} className="flex gap-2 items-center shrink-0">
          ...
          <input ref={inputRef} type="text" value={prompt} onChange={(e) => setPrompt(e.target.value)}
            placeholder={submitting ? "Waiting for Game Master..." : cooldown ? "Cooldown..." : "Describe your action..."}
          ...
          <button type="submit" disabled={submitting || cooldown || !prompt.trim()} className="terminal-btn text-sm">SEND</button>
        </form>
      )}
      ...
      {battle.status !== "active" && comicPhase === "idle" && (
        <div className="shrink-0 text-center py-3">
          <p className={`text-lg mb-3 ${battle.status === "victory" ? "text-green glow-green" : battle.status === "defeat" ? "text-red glow-red" : "text-amber glow-amber"}`}>
            {battle.status === "victory" ? `★ ${enemyName} has been defeated! ★` : battle.status === "defeat" ? "✖ You have fallen... ✖" : "⏱ Battle timed out! ⏱"}
          </p>
          <button onClick={() => onBattleEnd({ status: battle.status as "victory" | "defeat" | "timeout", score: battle.score, turns: battle.turn, enemyName })}
            className="terminal-btn text-green glow-green text-lg px-6 py-2">{"> VIEW RESULTS <"}</button>
        </div>
      )}
    </div>
  );
